...
">{{ $report['name'] ?? '' }} ({{ substr($report['cnic'], -4) }})</td>
        ...
        <td><input type="number" class="form-control labour_amount" name="labour_amount[]"  data-id="{{ $report['id'] }}" ></td>
    </tr>
@endforeach
<tr>
    <td colspan="8" style="text-align:right;">Total</td>
    <td>{{ $total_amount ?? '' }}</td>
    <td>@if($personWiseReports->count() > 0)<button class="btn ghost" id="createLabourVoucher">Create Labour Voucher</button>@endif</td>
</tr>
