    ...
    <body>
            ...
            <tbody>
                <tr>
                    <td colspan="3" style="padding:4px 12px;width:40%;vertical-align:top">
                        ...
                        <div style="margin-left: 10px">
                            <span style="display: inline-block; width: 100px;" >Total Received:</span>&nbsp;&nbsp;<span>{{ Setting::formatAmount($recovery)  }}<i class="fa fa-address-book" aria-hidden="true"></i></span>
                        </div>
        ...
        <table dir="@if( Config::get('app.locale') == 'ar' ){{'rtl'}}@endif" style="width: 100%;border-collapse: collapse;">
            <tr class="table-header" style="background-color: rgb(1, 75, 148); color: white;">
                ...
                <td style="border:1px dotted #222;padding:1px 3px;width:35%;text-align:center">Discription</td>
                <td style="border:1px dotted #222;padding:1px 3px;width:9%;text-align:center">Installment Amount</td>
                <td style="border:1px dotted #222;padding:1px 3px;width:9%;text-align:center">Recover</td>
                <td style="border:1px dotted #222;padding:1px 3px;width:9%;text-align:center">Due Amount</td>
            
            </tr>
            @php
                $grandTotal = $old_recovery = 0;
            @endphp

            @foreach ($data->bookingDetails as $d )
            <tr>
                <td style="@if( Config::get('app.locale') == 'ar' ){{'border-right:1px dotted #222;'}}@endif border:1px dotted #222;padding:1px 3px;text-align: center;">{{ $d->due_date }}</td>
                <td style="border:1px dotted #222;padding:1px 3px;font-size: 10px;line-height: 1.2;">
                    {{ ucfirst($d->installment_details) }}

                </td>
               
                <td style="border:1px dotted #222;border-right:1px solid #222;padding:1px 3px;text-align:center;font-size: 10px;">{{ Setting::roundformatAmount($d->amount)  }}</td>
                <td style="border:1px dotted #222;border-right:1px solid #222;padding:1px 3px;text-align:center;font-size: 10px;">
                @if ($recovery >= $d->amount)
                    {{ Setting::roundformatAmount($d->amount)  }} 
                    @php
                        $old_recovery = $recovery;
                        $recovery = $recovery - $d->amount;
                    @endphp
                @elseif ($recovery > 0)
                {{ Setting::roundformatAmount($recovery)  }} 
                @php
                        $recovery =  $d->amount - $recovery;
                        $recovery = 0;
                @endphp
                ...
                <td style="border:1px dotted #222;border-right:1px solid #222;padding:1px 3px;text-align:center;font-size: 10px;">
                    @if ($old_recovery >= $d->amount)
                        0.00
                    @else
                        {{ Setting::roundformatAmount($d->amount - $old_recovery)  }} 
                    @endif
                    @php
                        $old_recovery = $recovery;
                    @endphp  
            ...
            <tr>
                <td></td>
                <td><b>Total</b></td>
                <td><b>{{ Setting::roundformatAmount($grandTotal)  }}</b></td>
                <td> <b>{{ Setting::roundformatAmount($total_recovery)  }}</b>  </td>
                <td> {{ Setting::roundformatAmount($grandTotal - $total_recovery)  }} </td>
            </tr>
