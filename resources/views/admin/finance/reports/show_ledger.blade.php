    ...
    $(document).ready(function() {
        ...
        $('#your-datatable-id').DataTable({
            ...
            footerCallback: function (row, data, start, end, display) {
                ...
                var amountInSumPage = api.column(6, { page: 'current' }).data().reduce(function (acc, value) {
                    return acc + parseFloat(value);
                }, 0);

                // Calculate the sum of the "amount_out" column for the current page
                var amountOutSumPage = api.column(7, { page: 'current' }).data().reduce(function (acc, value) {
                    return acc + parseFloat(value);
                }, 0);

                // Calculate the sum of the "amount_in" column for all pages
                var amountInSumAll = api.column(6 , { search: 'applied' }).data().reduce(function (acc, value) {
                    return acc + parseFloat(value);
                }, 0);

                // Calculate the sum of the "amount_out" column for all pages
                var amountOutSumAll = api.column(7 ,{ search: 'applied' }).data().reduce(function (acc, value) {
                    return acc + parseFloat(value);
                }, 0);


                // Set the sum in the footer
                $(api.column(6, { page: 'current' }).footer()).html('Cash in: ' + amountInSumPage.toFixed(2));
                $(api.column(7, { page: 'current' }).footer()).html('Cash out: ' + amountOutSumPage.toFixed(2));
